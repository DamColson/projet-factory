...
class AdminInit {
	...
	// ATTENTION JEUNE MARGOULIN, LES TESTS QUI SUIVENT SONT A REALISER DANS UN
	// ORDRE SPECIFIQUE, RETIRE DONC LES @Disabled REQUIS POUR LA PREMIERE SEQUENCE
	// DE TEST, EFFECTUE LES TESTS, REATTRIBUE LES @Disabled
	// SUR LA PREMIERE SEQUENCE DE TESTS AINSI EFFECTUES ET RETIRE LES @Disabled DE
	// LA SECONDE SEQUENCE DE TESTS. UNE FOIS CELA FAIT, REATTRIBUE LES @Disabled
	// SUR LA SECONDE SEQUENCE DE TEST ET TU SERAS PRET
	...
	void initCompteTechnicien() {
		for (int i = 0; i < 3; i++) {
			...
			tech.setPassword("technicien" + i + "A1.");
	...
	void initCompteGestionnaire() {
		for (int i = 0; i < 3; i++) {
			...
			gest.setPassword("gestionnaire" + i + "A1.");
	...
	void initCompteFormateur() {
		for (int i = 0; i < 3; i++) {
			...
			formateur.setPassword("formateur" + i + "A1.");
	...
	void initCompteStagiaire() {
		for (int i = 0; i < 3; i++) {
			...
			stagiaire.setPassword("stagiaire" + i + "A1.");
	...
	void initVideoProjecteur() {
		for (int i = 0; i < 3; i++) {
			...
			videoProjecteur.setDateAchat(LocalDate.now());
	...
	@Commit
	void initStagiaire() {
		List<Compte> comptes = compteSrv.getByRole(Role.ROLE_STAGIAIRE);
		List<Ordinateur> ordinateurs = ordinateurSrv.getAllDisponible();
		for (int i = 0; i < comptes.size(); i++) {
			Compte compte = comptes.get(i);
